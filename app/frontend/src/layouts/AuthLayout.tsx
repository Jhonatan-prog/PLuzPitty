...
const AuthLayout = memo(({title, reference, children}: AuthComponentPropsType) => {
    ...
    return (
        <Fragment>
            ...
            <div className={`${TSCSS.gridCenter} md:grid-cols-2 h-[100vh] w-full overflow-hidden`}>
                <div className={reference + "-page " + `${TSCSS.flexStart} max-h-[600px] items-center pt-3`}>
                    <h2 className="title text-4xl text-[#1F271B] pb-5 w-full text-center">{title}</h2>
                ...
                {isLargeScreen && (
                    <div className="lib-image relative w-full h-full">
                        <img
                            className={TSCSS.transformCenter + " min-w-[500px] w-full h-auto max-w-[700px]"}
                            src={AuthImage}
